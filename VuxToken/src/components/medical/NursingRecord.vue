...
<group style="margin-top: 3em;" label-width="5.5em" label-margin-right="1em"  label-align="left"  >
      ...
      <x-header  class="vux-header-box"  style="background:linear-gradient(to right, #447bba  , #7a88bd,  #928dba);color:black;" >
          救治记录<a slot="right" style="color:black "   @click="sumbit()">保存</a> 
  ...
  <div>
   ...
   <cell :title="('执行时刻')"> 
      <el-date-picker v-model="DataModel.form.DoTime" @focus="forbid" size="large"  type="datetime"  placeholder="选择执行时刻"  default-time="12:00:00">
      </el-date-picker></cell>
  ...
  <div v-transfer-dom>
      <popup v-model="ShowEditGoods" height="28em" :hide-on-blur=false is-transparent >
        ...
        <group>
          ...
          <cell title="物品规格" :value="GoodModel.form.Spec"></cell>
           <x-input   title="数量"      v-model="GoodModel.form.Num"   text-align="right"  @on-blur="NumRule1"  @on-change="ChangeUseNum()"  ></x-input>
           <x-input   title="使用剂量"  v-model="GoodModel.form.UseNum" text-align="right"></x-input>
           <x-input   title="意外消耗"  v-model="GoodModel.form.LossVal" text-align="right"
            @on-blur="NumRule"
           ></x-input>
         </group>
    ...
    <el-table
            ...
            <el-table-column    width="40" label="删除">
              <template slot-scope="scope">
                 <img slot="icon" src="../../assets/images/icons/Delete.png"  @click="DeleteHC( scope.row)"> 
